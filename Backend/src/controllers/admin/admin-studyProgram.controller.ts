...
import type { ParamProgramStudyId, CreateStudyProgram, GetAllProgramStudy, UpdateProgramStudy } from '@/validations/admin.validation.js';
...
class AdminStudyProgramController {
  static createProgramStudy = catchAsync(async (c: Context) => {
    const { code, name, facultyId, degree, accreditation } = c.get('parsedJson') as CreateStudyProgram;
  ...
  static getAllProgramStudy = catchAsync(async (c: Context) => {
    const options = c.get('parsedQuery') as GetAllProgramStudy
  ...
  static updateProgramStudy = catchAsync(async (c: Context) => {
    const { programStudyId } = c.get('parsedParam') as ParamProgramStudyId;
    const body = c.get('parsedJson') as UpdateProgramStudy;
